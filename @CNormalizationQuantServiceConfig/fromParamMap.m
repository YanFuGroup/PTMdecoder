...
base_cfg_struct.min_xic_nonzero_points = CParamMapUtils.getOptionalNumber(task_param_map, CPTMdecoderWorkflowParamKeys.PARAM_MIN_XIC_NONZERO_POINTS, 5, 'CNormalizationQuantServiceConfig');
if ~isscalar(base_cfg_struct.min_xic_nonzero_points) || ~isfinite(base_cfg_struct.min_xic_nonzero_points) || ...
        base_cfg_struct.min_xic_nonzero_points < 1 || floor(base_cfg_struct.min_xic_nonzero_points) ~= base_cfg_struct.min_xic_nonzero_points
    CLogger.error(['[CNormalizationQuantServiceConfig:InvalidMinXicNonzeroPoints] ', ...
