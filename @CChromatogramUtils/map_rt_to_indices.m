...
%   peak_ranges:    Structure array with .left_bound and .right_bound (Index values)
    ...
    num_iso = length(final_XIC_peak);
    peak_ranges = repmat(struct('left_bound',0,'right_bound',0), num_iso, 1);
    
    for idx_iso = 1:num_iso
        if is_skip_vec(idx_iso), continue; end
        
        [diff_l, peak_ranges(idx_iso).left_bound] = min(abs(rt_grid - final_XIC_peak(idx_iso).left_bound));
        if diff_l > rt_error_tol
            error(['Cannot find the spectra on the specified retention time: ', ...
                num2str(final_XIC_peak(idx_iso).left_bound)]);
        end
        
        [diff_r, peak_ranges(idx_iso).right_bound] = min(abs(rt_grid - final_XIC_peak(idx_iso).right_bound));
        if diff_r > rt_error_tol
            error(['Cannot find the spectra on the specified retention time: ', ...
                num2str(final_XIC_peak(idx_iso).right_bound)]);
        end
    end
end
