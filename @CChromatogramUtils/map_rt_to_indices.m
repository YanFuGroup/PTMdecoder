function xic_peak_idx_bounds = map_rt_to_indices(xic_rt, xic_peak_rt_bounds, is_skip_vec, rt_error_tol)
% map_rt_to_indices
...
%       Structure array with .idx_start and .idx_end (index values into xic_rt)
    ...
    num_imp = length(xic_peak_rt_bounds);
    ...
    for idx_imp = 1:num_imp
        ...
        [diff_l, xic_peak_idx_bounds(idx_imp).idx_start] = min(abs(xic_rt - xic_peak_rt_bounds(idx_imp).rt_start));
        if diff_l > rt_error_tol
            error(['Cannot find the spectra on the specified retention time: ', ...
            num2str(xic_peak_rt_bounds(idx_imp).rt_start)]);
        end
        
        [diff_r, xic_peak_idx_bounds(idx_imp).idx_end] = min(abs(xic_rt - xic_peak_rt_bounds(idx_imp).rt_end));
        if diff_r > rt_error_tol
            error(['Cannot find the spectra on the specified retention time: ', ...
            num2str(xic_peak_rt_bounds(idx_imp).rt_end)]);
        end
    end
end
