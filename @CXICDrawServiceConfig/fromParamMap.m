...
for i_key = 1:length(required_keys)
    ...
    if ~task_param_map.isKey(key_name)
        error('CXICDrawServiceConfig:MissingConfigField', ...
...
if task_param_map.isKey('checked_peptides_res_path')
    cfg.checked_peptides_res_path = task_param_map('checked_peptides_res_path');
else
