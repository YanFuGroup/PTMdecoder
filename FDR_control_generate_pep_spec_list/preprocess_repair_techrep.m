...
% 9 mods (light only)
res_path = 'D:\research\project\Mixspec_code\Histone\human_histone_repair_techrep\mascot_dat';
work_dir_res = 'D:\research\project\Mixspec_code\Histone\human_histone_repair_techrep\result';
...
%     write_mascot_result_table(FDR_filtered_result, filtered_fileName);
    ...
    % Select the modified peptides without Oxidation, cannot be all fixed
    %   modification
    ...
    for i_final = 1:length(unique_filtered_result)
        if ~isequal(unique_filtered_result(i_final).modification,'-')...
                ...
                && ~all(ismember(strsplit(unique_filtered_result(i_final).modification, ','), ...
                    {'Label:13C(6) (K)','Label:13C(6)15N(4) (R)'}))
