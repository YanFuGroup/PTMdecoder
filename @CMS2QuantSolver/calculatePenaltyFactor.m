...
if nargin < 4 || isempty(case_penalty_intens)
    case_penalty_intens = 'intens_sum';
end
if nargin < 5 || isempty(grid_penalty_intens)
...
if Nb > 0 && Ny > 0 && sumInten_b > 0 && sumInten_y > 0
    form_score = log_factorial(Nb) + log_factorial(Ny) + log10(sumInten_b) + log10(sumInten_y);
else
    form_score = eps;
end
end