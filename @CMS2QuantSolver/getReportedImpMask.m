...
%   tau (1 x 1 double)
%       Absolute threshold computed as relative_threshold * max(abundance).
...
if max_abundance <= 0
	CLogger.error(['[CMS2QuantSolver:InvalidMaxAbundance] ', ...
		'max(abundance) must be positive for reported IMP selection.']);
end
