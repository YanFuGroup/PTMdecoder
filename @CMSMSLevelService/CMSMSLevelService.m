    ...
    methods
        ...
        function run(obj)
                ...
                if length(str)==1 || isempty(str{2})
                    ...
                    try
                        ...
                        [bSuccess,cstrIMP,abundance,ionTypePosCharge,ionIntens,frageff, ...
                            solver_diag,noise_model_fit_inputs,stability_cache] = ...
                        ...
                        CLogger.error('[CMSMSLevelService:run] Failed spectrum (%s/%s) - %s: %s', ...
            ...
            for idxRecord = 1:numel(stage1_records)
                ...
                spectrumMeta = struct( ...
                    'jaccard_stability', rec.solver_diag.jaccard_stability, ...
                    'vif_all_imp_max', rec.solver_diag.vif_all_imp_max, ...
                    'vif_reported_imp_max', rec.solver_diag.vif_reported_imp_max);
                msms_result.addSpectrum(rec.dataset_name, rec.spec_name, spectrumMeta);
                
                support_full = NaN(size(rec.abundance));
                vif_full = NaN(size(rec.abundance));
                ...
                vif_reported_each = rec.solver_diag.vif_reported_each;
                ...
                if ~isempty(reported_imp_indices) && ~isempty(vif_reported_each)
                    if numel(reported_imp_indices) ~= numel(vif_reported_each)
                        CLogger.error(['[CMSMSLevelService:InconsistentReportedVifLength] ', ...
                            'reported_imp_indices length (%d) must equal vif_reported_each length (%d) for %s/%s.'], ...
                            numel(reported_imp_indices), numel(vif_reported_each), rec.dataset_name, rec.spec_name);
                    end
                    out_of_range_mask = reported_imp_indices < 1 | reported_imp_indices > numel(vif_full);
                    if any(out_of_range_mask)
                        CLogger.error(['[CMSMSLevelService:InvalidReportedImpIndex] ', ...
                            'reported_imp_indices contains out-of-range entries for %s/%s.'], ...
                            rec.dataset_name, rec.spec_name);
                    end
                    vif_full(reported_imp_indices) = vif_reported_each;
                end
