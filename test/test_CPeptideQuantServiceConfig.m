function tests = test_CPeptideQuantServiceConfig
% Validate CPeptideQuantServiceConfig stability filter parsing.
% Inputs:
%    (none)
% Outputs:
...
f = @() CPeptideQuantServiceConfig.fromParamMap(param_map);
verifyLoggedErrorContains(testCase, f, 'CPeptideQuantServiceConfig:InvalidMinJaccardStability');
end
...
f = @() CPeptideQuantServiceConfig.fromParamMap(param_map);
verifyLoggedErrorContains(testCase, f, 'CPeptideQuantServiceConfig:InvalidMaxAbundanceMad');
end
...
% Build a minimal valid parameter map for peptide quant config.
% Inputs:
%    (none)
% Outputs:
%    param_map (containers.Map)
%        Valid key-value map for CPeptideQuantServiceConfig.
