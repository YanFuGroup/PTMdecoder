classdef test_CIMPQuantResultIO < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testWriteEmptyReportClosesOutput(testCase)
            tmp_out = [tempname, '.txt'];

            report = CIMPQuantReport();
            CIMPQuantResultIO.write(report, tmp_out);
            cleanup = onCleanup(@() delete(tmp_out));

            content = fileread(tmp_out);
            testCase.verifyTrue(contains(content, 'Protein_name,Peptide_start_position_on_protein;'));
            testCase.verifyTrue(contains(content, sprintf('*\tIMP\tCharge\tDataset')));
            testCase.verifyTrue(contains(content, sprintf('@\tRT_start\tRT_end\tProportion\tCheck_label')));

            parsed_report = CIMPQuantResultIO.read(tmp_out);
            testCase.verifyEmpty(parsed_report.blocks);
        ...
        function testSplitByDatasetRejectsSafeFilenameCollisions(testCase)
            tmp_dir = tempname;
            mkdir(tmp_dir);
            cleanup = onCleanup(@() rmdir(tmp_dir, 's'));
            input_path = fullfile(tmp_dir, 'report.txt');
            output_dir = fullfile(tmp_dir, 'split_by_dataset');
            ...
            lines = {
                ...
                sprintf('@\tRT_start\tRT_end\tProportion\tCheck_label')
                'sp|P1|PROT1,10;'
                sprintf('*\tIMP_A\t+2\tMCF7:DMSO_1.mgf\t500.0000\t499.500000\t500.500000\t100.000000')
                sprintf('@\t1.000000\t2.000000\t1.000000\t1')
                sprintf('*\tIMP_B\t+2\tMCF7_DMSO_1.mgf\t501.0000\t500.500000\t501.500000\t200.000000')
                sprintf('@\t3.000000\t4.000000\t1.000000\t0')
                };
            ...
            split_func = @() CIMPQuantResultIO.splitByDataset(input_path, output_dir, 'report');
            testCase.verifyError(split_func, 'CIMPQuantResultIO:SplitDatasetNameCollision');
